...
async function fetchWithRetry(url, retries = 3) {
    for (let i = 0; i < retries; i++) {
        try {
            ...
            if (blocked) throw new Error(`Blocked (${res.status})`);
    ...
    const wait = 1000 * Math.pow(2, i) + Math.random() * 300;
    await sleep(wait);
}
            }
        }
...
const $ = cheerio.load(html);
            ...
            const nextPage = $(".pagination .next:not(.disabled), .page-link[rel='next'], a[rel='next']");
